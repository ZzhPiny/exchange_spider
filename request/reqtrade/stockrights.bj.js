...
var reqCentralData = path => new Promise((resolve, reject) => request(path).then(body => {
    ...
    var arrRequest = data.map((item, index) => callback => request(item.path).then(body => {
        ...
        commonInfo.startTime = $trListofInfo.eq(2).find('td').eq(1).text();
        commonInfo.endTime = $trListofInfo.eq(2).find('td').eq(3).text();
        ...
        var exchangeProject = [];
        exchangeByTransferParty.forEach(item => {
...
function getDataByTrList($trList) {
    ...
    $trList.each((index, tr) => {
        ...
        var date = tdList[4].children[0].data;
        console.log(date);
